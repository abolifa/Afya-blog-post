...
import { Phone, Copy, Check, Clock, MapPin, PhoneCall } from "lucide-react";
...
export default function ContactPhonePage() {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["phone"],
    queryFn: async () => {
      const res = await api.get("/contact/phone");
  ...
  return (
    ...
    >
      ...
      <nav className="text-sm text-muted-foreground mb-1">
        ...
        <Link href={`/${locale}/contact/phone`}>اتصل بنا (هاتف)</Link>
      </nav>

      {/* Header */}
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">الاتصال الهاتفي</h1>
        {data && <Badge variant="secondary">{data.length} أرقام</Badge>}
      </div>
        ...
        {data?.map((contact) => (
          <Card key={contact.id} className="border bg-card/80">
            ...
            <CardContent className="space-y-3">
              {contact.value.map((phoneNumber, idx) => (
                <div
                  key={`${contact.id}-${idx}`}
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button asChild size="sm">
                      <a
                        href={`tel:${phoneNumber}`}
                        aria-label={`اتصال بـ ${phoneNumber}`}
                      >
